...
const AboutUsSection = () => {
  ...
  return (
    <div className={isDark ? 'dark' : ''} id='about'>
      <section 
        ref={sectionRef}
        className={`relative py-10 lg:py-10 overflow-hidden transition-colors duration-500 ${
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
           {/* Blob 1 */}
           <div className={`absolute top-0 left-0 w-[500px] h-[500px] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob ${
             isDark ? 'bg-green-900' : 'bg-green-200'
           }`}></div>
           {/* Blob 2 */}
           <div className={`absolute bottom-0 right-0 w-[500px] h-[500px] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000 ${
        ...
        <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Header Section */}
          <div className={`text-center mb-16 max-w-3xl mx-auto transition-all duration-1000 transform ${
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`group relative p-6 rounded-2xl transition-all duration-700 transform hover:-translate-y-2 ${
                  hasAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                } ${
                  isDark 
              ...
              >
                <div className={`inline-flex items-center justify-center w-12 h-12 rounded-xl mb-4 transition-colors ${
                  isDark ? 'bg-gray-700 text-green-400 group-hover:bg-green-900/50' : 'bg-green-50 text-green-600 group-hover:bg-green-100'
                }`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
            ...
            <div className={`space-y-8 transition-all duration-1000 delay-300 ${
              ...
              <div className={`p-8 rounded-3xl ${
                isDark ? 'bg-gradient-to-br from-gray-800 to-gray-900' : 'bg-white shadow-xl shadow-green-900/5'
              }`}>
                  ...
                  {benefits.map((benefit, index) => (
                    <div key={index} className="flex items-center space-x-3 group">
                      <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-colors ${
                        isDark ? 'bg-green-900/50 text-green-400 group-hover:bg-green-500 group-hover:text-white' : 'bg-green-100 text-green-600 group-hover:bg-green-500 group-hover:text-white'
                      }`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {features.map((feature, index) => (
                <div
                  key={index}
                  className={`p-6 rounded-2xl transition-all duration-700 hover:scale-[1.02] ${
                    hasAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  } ${
                    isDark 
                ...
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-4 ${
                    isDark ? 'bg-gray-700 text-green-400' : 'bg-green-50 text-green-600'
                  }`}>
